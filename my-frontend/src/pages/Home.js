...
// Backend API base URL comes from centralized config
  ...
  useEffect(() => {
    const fetchFeaturedProperties = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error(
            `Error fetching properties: ${res.status} ${res.statusText}`
          );
        ...
        if (!contentType || !contentType.includes("application/json")) {
          console.error(
            `Expected JSON but got ${contentType}. Response:`,
            await res.text()
          );
  ...
  useEffect(() => {
    const fetchFavourites = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error(
            `Error fetching favourites: ${res.status} ${res.statusText}`
          );
          throw new Error(`Server returned ${res.status}: ${res.statusText}`);
        }
